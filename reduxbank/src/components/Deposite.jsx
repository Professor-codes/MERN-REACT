...
import { depositeMoney } from '../redux/BankSlice'
...
export const Deposite = () => {
  ...
  const depositeHandler = () => {
    ...
    dispatch(depositeMoney(parseInt(deposite)))
  }
  ...
  return (
    <div>
      ...
      <div>
        <label>Deposite money from here</label>
        <input type='text' placeholder='Enter amount to deposite' onChange={(e) => {
          setdeposite(e.target.value)
        }} />
        <button className='btn' onClick={() => { depositeHandler() }}>DEPOSITE</button>
      </div>
    </div>
